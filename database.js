...
const readDB = async (token) => {
    try {
        if(token === "eth"){
            const output = await fs.readFile("ethPrice.db.json", function(err, data) {
                if(err) throw err;
            ...
            const output = await fs.readFile("h2usdPrice.db.json", function(err, data) {
                if(err) throw err;
                return Buffer.from(data);  // In JavaScript, a buffer is a region of memory used to temporarily store data while it is being moved from one place to another. 
            });
            console.log(output);
...
const writeDB = async (price, time, lastEntry, token) => {
    try {
        ...
        if(token === "eth"){
            ...
            let output = await fs.writeFile("ethPrice.db.json", JSON.stringify(ethPriceDB), err => {
                if(err) throw err;
                return "Done" 
            });
            return output;
            ...
            let output = await fs.writeFile("h2usdPrice.db.json", JSON.stringify(h2usdPriceDB), err => {
                if(err) throw err;
                return "Done" 
            });
            return output;
